...
case 'lostpassword' :
	...
	if ( isset( $_REQUEST['error'] ) ) {
		if ( 'invalidkey' == $_REQUEST['error'] )
			$errors->add( 'invalidkey', __( '<b>错误：</b>请求无效,请您重新获取！' ) );
		elseif ( 'expiredkey' == $_REQUEST['error'] )
			$errors->add( 'expiredkey', __( '<b>错误：</b>请求已过期,请您重新获取！' ) );
	}
...
case 'resetpass' :
	...
	if( strlen($_POST['pass1'])<6 ) {  
		$errors->add( 'password_reset_mismatch2', '<b>温馨提示：</b>新密码不少于6位！' );
    }
...
<div class="content-wrap">
	<div class="content resetpass">
		...
		<ul class="resetpasssteps">
			<li<?php echo $classactive1 ?>>1.获取密码重置邮件<span class="glyphicon glyphicon-chevron-right"></span></li>
			<li<?php echo $classactive2 ?>>2.设置新密码<span class="glyphicon glyphicon-chevron-right"></span></li>
			<li<?php echo $classactive3 ?>>3.成功修改密码</li>
		</ul>
		...
		<?php 
				...
				echo '<h3><span class="text-success">已向注册邮件发送通知！</span></h3>';
				echo '<p><b><a href="https://mail.qq.com" target="_blank">点我</a></b>去邮箱查收邮件并点击重置密码链接</p>';
			}
		} ?>
		...
		<?php if( $classactive3 ){ ?>
		<form>
			<h3><center><span class="text-success"><span class="glyphicon glyphicon-ok-circle"></span> 恭喜，您的密码已重置！</span></center></h3>
			<p> &nbsp; </p>
			<div style="text-align:center"><a href="<?php echo get_bloginfo('url') ?>/index.php/user" target="_blank" style="text-decoration:none;color:#fff;display:inline-block;line-height:44px;font-size:18px;background-color:#61B3E6;border-radius:3px;font-family:Microsoft Yahei">&nbsp; &nbsp;&nbsp; &nbsp;马上登录&nbsp; &nbsp;&nbsp; &nbsp;</a><br><br></div>
		</form>
		<?php } ?>
...
function retrieve_password() {
	...
	if ( empty( $_POST['user_login'] ) ) {
		$errors->add('empty_username', __('<strong>错误</strong>: 您还没有输入用户名或电子邮件！'));
	...
	if ( !$user_data ) {
		$errors->add('invalidcombo', __('<strong>错误</strong>: 请输入有效的用户名或电子邮件！'));
		return $errors;
	}
	...
	$message .= sprintf(__('尊敬的【%s】您好：'), $user_login) . "\r\n\r\n";
	$message .= __('有人正在请求重置《小航工作室》的会员密码') . "\r\n\r\n";
	$message .= __('若这不是您本人操作的，请忽略此邮件。') . "\r\n\r\n";
	$message .= __('已经过我本人确认无误，打开链接完成密码重置：') . "\r\n\r\n";
	$message .= network_site_url(mo_get_user_rp() . "?action=resetpass&key=$key&login=" . rawurlencode($user_login), 'login') . "\r\n\r\n";
 $message .= __('如不能正常打开请复制到浏览器') . "\r\n\r\n";
 $message .= __('感谢您的支持与理解！');
